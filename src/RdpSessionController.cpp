                                 ...
                                 const QString &sharedFolder)
{
    ...
    // +clipboard turns RedirectClipboard on. direction-to/files-to only adjust
    // the feature mask — without +clipboard those options leave the channel off.
    args << QStringLiteral("+clipboard")
         << QStringLiteral("/clipboard:direction-to:all,files-to:all")
    ...
    setStatus(QStringLiteral("Starting FreeRDP (%1%, %2x%3) with %4…")
                  .arg(scale)
                  .arg(width)
                  .arg(height)
                  .arg(m_clientBinary));
...
void RdpSessionController::stopClipboardBridge()
{
    if (m_clipboardBridge)
